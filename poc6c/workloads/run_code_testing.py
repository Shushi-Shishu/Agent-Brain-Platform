"""CLI for manual or sub-agent POC 6c code-testing pilot runs."""
...
if __package__ in {None, ""}:
    # Keep the documented ``python workloads/run_code_testing.py`` entry point
    # working without requiring callers to set PYTHONPATH.
...
MATCHED_CONDITIONS = {
    ...
    "timeout_seconds_per_test_run": 3,
}
...
def _task(task_id: str):
    tasks = {task.task_id: task for task in load_code_testing_fixtures()}
    if task_id not in tasks:
        raise ValueError(f"unknown task: {task_id}")
    return tasks[task_id]


def _existing_pair(task_id: str, run_root: Path) -> PreparedCodeTestingPair:
    task = _task(task_id)
...
def prepare(args) -> int:
    pairs = []
    for task in load_code_testing_fixtures():
    ...
    print(f"prepared {len(pairs)} matched code-testing pairs")
    return 0
...
def complete(args) -> int:
    ...
    print(
        ...
        f"faults={metrics.get('distinct_faults_exposed')}/"
        f"{metrics.get('fault_count')} "
        f"coupled={bool(metrics.get('implementation_coupling_findings'))} "
        f"stable={metrics.get('stable')}"
    )
...
def summarize(args) -> int:
    reports = [
        ...
        for path in sorted(args.reports_root.glob("TST-P*-*.json"))
    ]
    _write_json(args.report_output, summarize_code_testing_reports(reports))
...
def parser() -> argparse.ArgumentParser:
    ...
    command.add_argument(
        "--agent-result", default="agent completed assigned test design"
    )
